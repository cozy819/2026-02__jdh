...
| 차시 | 파일 | 쪽 | A4 | 유형 | 상태 |
|---|---|---:|---:|---|---|
| — | `index.html` | 10 | 10 | 표지·목차 (기본 · 브랜딩본) | 완성 · 한중 |
| — | `index-nobrand.html` | 10 | 10 | 표지·목차 (회사 표기 없음) | 완성 · 한중 |
| 1-1 | `01-1.html` | 24 | 24 | 오리엔테이션 | 완성 · 한중 |
| 1-2 | `01-2.html` | 16 | 16 | 이론 | 완성 · 한중 |
| 2-1 | `02-1.html` | 26 | 26 | 이론 | 완성 · 한중 |
| 2-2 | `02-2.html` | 52 | 52 | 실습 | 완성 · 한중 |
| 3-1 | `03-1.html` | 38 | 38 | 이론 | 완성 · 한중 |
| 3-2 | `03-2.html` | 48 | 48 | 이론+실습 | 완성 · 한중 |
| 4-1 | `04-1.html` | 22 | 22 | 이론 | 완성 · 한중 |
| 4-2 | `04-2.html` | 20 | 20 | 실습(AI 요청·확인) | 완성 · 한중 |
| 5-1 | `05-1.html` | 22 | 22 | 평가 안내 | 완성 · 한중 |
| 5-2 | `05-2.html` | 25 | 25 | 수행평가 1(AI 활용) | 완성 · 한중 |
| 6-1 | `06-1.html` | 27 | 27 | 이론 | 완성 · 한중 |
| 6-2 | `06-2.html` | 23 | 23 | 활동(+양식 부록) | 완성 · 한중 |
| 7-1 | `07-1.html` | 25 | 25 | 이론 | 완성 · 한중 |
| 7-2 | `07-2.html` | 16 | 16 | 활동 | 완성 · 한중 |
| 8-1 | `08-1.html` | 14 | 14 | 평가 안내 | 완성 · 한중 |
| 8-2 | `08-2.html` | 17 | 17 | 수행평가 2 | 완성 · 한중 |
| 9-1 | `09-1.html` | 20 | 20 | 설계 활동 | 완성 · 한중 |
| 9-2 | `09-2.html` | 16 | 16 | 활동(AI 확정) | 완성 · 한중 |
| 10-1 | `10-1.html` | 13 | 13 | 이론(API·요청응답) | 완성 · 한중 |
| 10-2 | `10-2.html` | 10 | 10 | 실습(내부 API 연결) | 완성 · 한중 |
| 11-1 | `11-1.html` | 15 | 15 | 이론 | 완성 · 한중 |
| 11-2 | `11-2.html` | 13 | 13 | 실습(SQLite) | 완성 · 한중 |
| 12-1 | `12-1.html` | 18 | 18 | 이론(+양식 부록 5쪽) | 완성 · 한중 |
| 12-2 | `12-2.html` | 14 | 14 | 실습(테스트·시험 제출) | 완성 · 한중 |
| 13-1 | `13-1.html` | 13 | 13 | 수행평가 3 (수정) | 완성 · 한중 |
| 13-2 | `13-2.html` | 12 | 12 | 수행평가 3 (제출) | 완성 · 한중 |
| 9-0 | `별도배포/09-0.html` | 9 | 9 | AI 개발 조건 | 완성 · 한중 |
| 9-13 | `별도배포/09-13.html` | 24 | 24 | 교사용 준비자료 | 완성 · 한국어 |

**‘쪽’과 ‘A4’는 항상 같다.** `.page` 하나가 인쇄 한 장에 정확히 들어가도록 전체를 재분할했고, 한국어·중국어 두 상태 모두 헤드리스 크롬으로 렌더링해 확인했다. 13회 26개 차시 전체가 집필 완료되었고, 합계 **559쪽**이다(`index.html`·`index-nobrand.html`·별도배포 제외). **2-2(52쪽)와 3-2(48쪽)가 가장 길다.** 3-2는 이론(리스트·튜플·조건·반복·함수·딕셔너리)과 통합 실습을 한 차시에 합친 결과다. 수업 운영에서 부담이 되면 오개념 점검과 확인 문제 뒤가 이론/실습 경계이므로 거기서 잘라 두 차시로 나눌 수 있다.

수행평가 1은 데이터 분석 능력보다 **AI를 이용해 교사가 지정한 그래프를 정확히 완성하는 과정**을 평가한다. 배점은 그래프 완성 50점, AI 활용과 학생의 판단 30점, 결과 확인과 간단한 설명 20점이다. AI 결과가 처음부터 조건에 맞으면 억지로 고치지 않고, 확인한 조건과 그대로 채택한 이유를 기록한다.

HTML이 참조하는 그림 16개는 모두 `assets/img/`에 들어 있다. 2026-08-24에 3-2~5-2용 도식 7개를 추가해 점선 자리표시자를 모두 해소했고, 같은 날 예전 인디고 팔레트로 남아 있던 6개를 **VCL 팔레트로 다시 칠했다** — 선은 `#6C7078`, 강조 채움은 `#FF5C00`, 둘째 강조는 `#0E7C86`, 둥근 모서리(`rect rx`)는 제거. 화살촉은 `markerUnits="userSpaceOnUse"`로 24px에 고정해 선 굵기에 따라 커지지 않게 했다. (`01-2-01-human-computer`와 `02-2-01-vscode-4`는 SVG 원본이 없어 그대로 둔다.) **6~13회에는 그림이 하나도 없다.** 설계 원고가 “화면은 그림 대신 글로 설명한다”를 명시하므로 의도된 것이며, 이 범위에는 자리표시자도 만들지 않았다.

> [!note] 폐기한 그림은 `assets/img/_retired/`로 옮긴다
> `01-2-02-cli-gui` — 1-2에서 CLI→GUI 절이 삭제되면서 폐기.
> `01-1-01-literacy` — 정보·문해력·디지털 문해력·AI 문해력을 동심원으로 그렸으나 **포함 관계가 뒤집혀 있었고**(문해력이 가장 바깥이어야 한다) **대상(정보)과 능력(문해력)을 한 축에 섞은 범주 오류**였다. 낱말 상자의 `경계` 줄이 같은 일을 더 정확하게 하므로 그림 없이 글로만 둔다.

## 2. 폴더 구조

```
교과서_배포/
├─ index.html          표지 + 전체 목차 + 읽는 법 — 기본(바이브코딩연구소 브랜딩)
├─ index-nobrand.html  같은 내용, 표지에서 회사 표기만 뺀 판
├─ 01-1.html … 13-2.html   차시별 교과서 (1~13회, 26개 차시 전체 완료)
├─ assets/
│  ├─ textbook.css     공용 디자인 (모든 차시가 공유)
│  ├─ textbook.js      모드 전환·목차·언어·복사·이미지 대체
│  ├─ i18n.py          번역 단위 추출/주입 도구 (§8)
│  └─ img/             그림 (파일명 = 이미지 ID)
├─ 별도배포/           따로 전달하는 자료 — 9-0 AI 개발 조건 · 교사용 준비자료
├─ 이미지_제작_가이드.md
└─ README.md
```
...
1. 가장 비슷한 유형의 기존 파일을 복사한다. (이론 → `07-1.html`, 활동 → `07-2.html`, 평가 안내 → `08-1.html`, 수행평가 → `08-2.html`)
...
수행평가 2 양식([[08-0 수행평가 2 양식 - 서비스 제작 계획서(PRD)]])의 **전문은 `06-2.html`의 부록 두 쪽**(`#s12`·`#s13`)에 실려 있다. 학생이 처음 양식을 여는 차시가 6-2이기 때문이다. `08-1.html`은 그 양식을 다시 싣지 않고 항목별 확인 기준만 다루므로, **양식을 고치면 `06-2.html` 부록도 함께 고친다.**

`05-2.html`은 이 원칙에 따라 정답 값을 뺀 상태다. 제출 규격(파일명 `05_assessment1_학생ID_이름.ipynb`, Notebook + 사용한 CSV를 ZIP 없이 같은 제출 화면에 올리기, CSV는 상대경로)은 [[플랫폼_설계/prd]]의 제출 규격과 맞물려 있으므로 한쪽을 고치면 다른 쪽도 확인한다. 8-1·8-2(수행평가 2)는 원고에 정답 블록이 없어 해당 사항이 없었다. 앞으로 13-1·13-2(수행평가 3)를 만들 때도 같은 표시를 확인한다. 배포 전 점검은 다음 한 줄로 한다.
...
python3 assets/i18n.py extract 06-1.html > /tmp/units.txt
...
# 3) 되넣는다
python3 assets/i18n.py apply 06-1.html /tmp/zh-06-1.txt
```
