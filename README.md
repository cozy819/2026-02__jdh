...
| 2-1 | `02/textbook/02-1.html` | 21 | 설치·확인 | 실제 결과 예시 → Python → OpenCode(Node.js는 설치 수단) → 설치 확인·PATH 오류 읽기와 복구 → VS Code·Python 확장 → 제공자·모델 연결과 첫 확인, CLI는 확인·대화 용도 |
| 2-2 | `02/textbook/02-2.html` | 18 | 기초 실습 | 숫자·문자·print·변수·기본 산술연산만 사용 |
| 2-3 | `02/textbook/02-3.html` | 11 | 기초 이론 | 주소에서 출발해 인터넷·웹·서버와 프로그래밍 언어의 역사를 살펴보는 흐름 |
| 3-1 | `03/textbook/03-1.html` | 30 | 설치+실습 | uv·Jupyter 확장·커널 연결, 비교연산, 문장→비교식 워크시트 |
| 3-2 | `03/textbook/03-2.html` | 23 | 자료구조+실습 | list·dict, 번호가 0부터인 이유, 정보 구조 분류 워크시트 2종 |
| 4-1 | `04/textbook/04-1.html` | 19 | 그래프 이해 | `pyproject.toml` 읽기, 오류를 AI에게 전달하기, Excel 원본을 읽고 그래프 결과 검토 |
| 4-2 | `04/textbook/04-2.html` | 21 | 표 데이터 실습 | 같은 자료의 CSV를 pandas로 읽기, AI 요청 5단계, 설명용 Notebook의 코드 블록 1~6번 읽기 |
| 5-1–13-2 | 해당 HTML | 343 | 평가·기획·구현 | 6·7·9~12회 재구성 — 내용은 그대로 두고 「지금 적는다」·워크시트를 끼워 손 움직이는 절을 늘렸다 (09-0·용어집 제외) |
| 9-0 | `09/textbook/09-0.html` | 9 | AI 개발 조건 | 학생용 제약 안내 |
| 9-13 | `별도배포/09-13.html` | 24 | 교사용 | 개발 수업 준비자료 |

**2-1–4-2는 스크롤·슬라이드·인쇄에서 같은 HTML을 사용한다.** 2-1은 21쪽, 2-2는 18쪽, 2-3은 11쪽, 3-1은 30쪽, 3-2는 23쪽, 4-1은 19쪽, 4-2는 21쪽이다. 2-1은 학생이 교과서만 보고 설치·확인을 따라갈 수 있도록 VS Code 화면·Python 확장·OpenCode 오류 대응·제공자 연결·모델 선택을 별도 쪽으로 분리했다. 인쇄 장수는 배포 직전 다시 확인한다.
...
- downloads: `NN_student_files.zip` 형식. 압축을 풀면 `NN/` 폴더 하나가 나오고, 학생은 그 폴더를 `JDH_VibeCoding_자기이름` 안에 넣는다. `03_student_files.zip`은 `pyproject.toml`·`uv.lock`을 포함하므로 학생이 `03` 폴더를 통째로 열고 `uv sync`를 실행하면 그대로 준비가 끝난다. `.venv`는 넣지 않는다 — 만드는 것이 3-1의 실습이다.
